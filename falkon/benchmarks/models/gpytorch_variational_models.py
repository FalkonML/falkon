...
def _choose_var_dist(dist_str, num_points, batch_shape=1):
    ...
    if dist_str == "diag":
        ...
        return gpytorch.variational.TrilNaturalVariationalDistribution(
            num_points, batch_shape=batch_shape)
    else:
...
def _choose_var_strat(model, var_strat, var_dist, ind_pt, learn_ind=True, num_classes=None):
    if var_strat == "multi_task":
        ...
        return UnwhitenedVariationalStrategy(model, ind_pt, var_dist,
...
class GenericApproxGP(BaseModel):
    def __init__(self,
        ...
        strategy = _choose_var_strat(
            self, var_strat, distribution, inducing_points, learn_ind=learn_ind_pts,
        ...
        if not strategy.variational_params_initialized.item():
            try:
                strategy._variational_distribution.initialize_variational_distribution(
                    strategy.prior_distribution)
...
class MultiTaskApproxGP(BaseModel):
    def __init__(self,
                 ...
                 batch_shape):
        distribution = _choose_var_dist(
            var_distrib, inducing_points.size(-2), batch_shape).cuda()
        strategy = _choose_var_strat(
            self, var_strat, distribution, inducing_points,
            learn_ind=learn_ind_pts, num_classes=batch_shape).cuda()
...
class GPTrainer:
    ...
    def do_train(self, Xtr, Ytr, Xval, Yval):
        ...
        if self.mb_size == 1:
            ...
            train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=self.mb_size,
        ...
        for epoch in range(self.num_epochs):
            ...
            for j, (x_batch, y_batch) in enumerate(train_loader):
                ...
                if j % self.error_every == 0:
                    ...
                    err, err_name = self.err_fn(y_batch.cpu(),
                                                self.model.likelihood(output).mean.detach().cpu())
    ...
    def predict(self, X):
        ...
        if self.mb_size > 1:
            test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=self.mb_size,
...
class RegressionVGP(GPTrainer):
    def __init__(self,
        ...
        super().__init__(model, err_fn, mb_size, use_cuda, mll=loss_fn, num_epochs=num_epochs,
                         lr=lr, params=params,
    ...
    def __str__(self):
        ...
        lengthscale = [p for name, p in dict(ker.named_parameters()).items() if
                       'raw_lengthscale' in name]
        num_ker_params = lengthscale[
            0].shape  # dict(ker.named_parameters())['raw_lengthscale'].shape
