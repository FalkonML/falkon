...
from cuda.initialization import cublas_handle
from falkon.cuda.cublas_gpu import cublasStrsm, cublasDtrsm
...
def cuda_trsm(A: torch.Tensor, v: torch.Tensor, alpha: float, lower: int, transpose: int) -> torch.Tensor:
    ...
    if is_f_contig(v, strict=False):
        ...
        vF = cuda_transpose(input=v, output=vF)
    ...
    if not is_f_contig(v, strict=False):
        ...
        vout = cuda_transpose(input=vF, output=vout)
        return vout
    return vF
