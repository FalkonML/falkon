...
def init_model(model_type: str,
    ...
    if model_type == "sgpr":
        model = SGPR(kernel=kernel, penalty_init=penalty_init, centers_init=centers_init,
                     opt_penalty=opt_penalty, opt_centers=opt_centers)
    elif model_type == "gcv":
        model = GCV(kernel=kernel, penalty_init=penalty_init, centers_init=centers_init,
                    opt_penalty=opt_penalty, opt_centers=opt_centers, )
    elif model_type == "loocv":
        model = LOOCV(kernel=kernel, penalty_init=penalty_init, centers_init=centers_init,
                      opt_penalty=opt_penalty, opt_centers=opt_centers)
        ...
        model = HoldOut(kernel=kernel, penalty_init=penalty_init, centers_init=centers_init,
                        opt_centers=opt_centers, opt_penalty=opt_penalty,
                        val_pct=val_pct, per_iter_split=per_iter_split)
    elif model_type == "creg-notrace":
        model = CompReg(kernel=kernel, penalty_init=penalty_init, centers_init=centers_init,
                        opt_penalty=opt_penalty, opt_centers=opt_centers)
    elif model_type == "creg-penfit":
        model = NystromCompReg(kernel=kernel, penalty_init=penalty_init, centers_init=centers_init,
                               opt_penalty=opt_penalty, opt_centers=opt_centers)
    elif model_type == "stoch-creg-penfit":
        model = StochasticNystromCompReg(kernel=kernel, penalty_init=penalty_init,
