...
torch::Tensor square_norm_cuda(const torch::Tensor& input, int64_t dim, torch::optional<bool> opt_keepdim) {
    ...
    printf("At beginning of cuda sq-norm: %.4fBytes\n", device_stats.allocated_bytes[static_cast<size_t>(
        c10::cuda::CUDACachingAllocator::StatType::AGGREGATE)].current);
    ...
    device_stats = c10::cuda::CUDACachingAllocator::getDeviceStats(0);
    printf("Before output creation: %.4fBytes\n", device_stats.allocated_bytes[static_cast<size_t>(
        c10::cuda::CUDACachingAllocator::StatType::AGGREGATE)].current);
    torch::Tensor result = torch::empty(result_shape, input.options());
    printf("After output creation: %.4fBytes\n", device_stats.allocated_bytes[static_cast<size_t>(
    ...
    device_stats = c10::cuda::CUDACachingAllocator::getDeviceStats(0);
    printf("After iterator creation: %.4fBytes\n", device_stats.allocated_bytes[static_cast<size_t>(
        c10::cuda::CUDACachingAllocator::StatType::AGGREGATE)].peak);
