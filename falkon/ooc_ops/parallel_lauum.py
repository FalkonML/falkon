        ...
        for b in range(len(block_allocs)):
            ...
            for r in my_rows:
                if r == b:
                    ...
                    cublas_trmm(A=col_r, lda=col_r.stride(1), alpha=1.0,
                                B=ccb, ldb=ccb.stride(1),
                                C=ccb, ldc=ccb.stride(1),
                    ...
                    if br.end < N:
                        cublas_gemm(A=col_r[br.length:, :], lda=col_r.stride(1), alpha=1.0,     # A is k * m
                                    B=ccb[br.length:, :], ldb=ccb.stride(1),                    # B is k * n
                                    C=ccb, ldc=ccb.stride(1), beta=1.0,                         # C is m * n
        ...
        for b in range(len(block_allocs)):
            ...
            for r in my_rows:
                ...
                if r == b:
                    ...
                    # C = alpha * op(A) @ B -- A triangular
                    cublas_trmm(A=whole_col_r, lda=max_block_size, alpha=1.0,
                                B=ccb, ldb=max_block_size,
                                C=ccb, ldc=max_block_size,
